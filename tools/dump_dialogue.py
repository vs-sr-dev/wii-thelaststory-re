"""Dump leggibile di un file di dialogo (game_message) in IT ed EN."""
import os
import sys, io
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from lwpack import parse_pfs, parse_pkh, path_hash, lz11_decompress

stem = os.path.join(os.environ.get('TLS_ROOT', '.'), r'extract\files\pack\filesystem')
...
out = io.open(os.path.join(os.environ.get('TLS_ROOT', '.'), r'tools\dialogue_sample.txt'), 'w', encoding='utf-8')
for name in ['game_message/dg001_01_it.u16', 'game_message/dg001_01_en.u16',
             'game_message/dg001_01_jp.u16']:
    t = get_text(name)
    rows = t.replace('\r\n', '\n').split('\n')
    out.write(f'===== {name} : {len(rows)} righe =====\n')
    for r in rows[:25]:
        out.write(r + '\n')
    out.write('\n')
out.close()
print('scritto dialogue_sample.txt')
