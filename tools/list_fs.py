"""Lista e statistiche del contenuto di filesystem/levels/eventpacks pack."""
import os
import sys, os, collections
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from lwpack import parse_pfs, parse_pkh

FILES = os.path.join(os.environ.get('TLS_ROOT', '.'), r'extract\files')
for name in ('filesystem', 'levels', 'eventpacks'):
    ...
    print(f'== {name}: {len(files)} nomi, {len(entries)} entry pkh, match={len(files)==len(entries)}')
...
# cerca file di testo/lingua in filesystem
...
print(f'\n{len(hits)} file "linguistici" in filesystem:')
for h in hits[:80]:
    print('  ', h)
if len(hits) > 80:
    print(f'   ... (+{len(hits)-80})')
